...
contract FakeUser {
  ...
  function doWethJoin(address obj, address gem, address urn, uint wad) public {
    WETH(obj).approve(address(gem), type(uint).max);
  ...
  function doTake(address obj, uint id, uint amt, uint max, address who, bytes calldata data)
    external
  {
  ...
  function doESMJoin(address gem, address esm, uint wad) public {
    DSToken(gem).approve(esm, type(uint).max);
    ESM(esm).join(wad);
  }
}
