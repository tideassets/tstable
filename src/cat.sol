...
interface Kicker {
  function kick(address urn, address gal, uint tab, uint lot, uint bid) external returns (uint);
}
...
contract Cat {
  ...
  uint public litter; // Balance of Dai out for liquidation [rad]

  // --- Events ---
  event Bite(
    bytes32 indexed ilk, address indexed urn, uint ink, uint art, uint tab, address flip, uint id
  );
  ...
  function bite(bytes32 ilk, address urn) external returns (uint id) {
...

    {
      ...
      uint tab = mul(mul(dart, rate), milk.chop) / WAD;
