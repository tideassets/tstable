...
interface SourceLike {
  function cure() external view returns (uint);
}
...
contract Cure {
  ...
  uint public wait;
  uint public when;
  mapping(address => uint) public pos; // position in srcs + 1, 0 means a source does not exist
  mapping(address => uint) public amt;
  mapping(address => uint) public loaded;
  uint public lCount;
  uint public say;
  ...
  function tCount() external view returns (uint count_) {
    count_ = srcs.length;
  }
  ...
  function load(address src) external {
    ...
    uint oldAmt_ = amt[src];
    uint newAmt_ = amt[src] = SourceLike(src).cure();
