...
contract DssProxyActions is Common {
  ...
  function convertTo18(address gemJoin, uint amt) internal returns (uint wad) {
    // For those collaterals that have less than 18 decimals precision we need to do the conversion before passing to frob function
    // Adapters will automatically handle the difference of precision
    wad = mul(amt, 10 ** (18 - GemJoinLike(gemJoin).dec()));
  }

  function _getDrawDart(address vat, address jug, address urn, bytes32 ilk, uint wad)
    internal
    returns (int dart)
  {
  ...
  function _getWipeDart(address vat, uint dai, address urn, bytes32 ilk)
    internal
    view
    returns (int dart)
  {
  ...
  function lockGem(address manager, address gemJoin, uint cdp, uint amt, bool transferFrom) public {
  ...
  function freeGem(address manager, address gemJoin, uint cdp, uint amt) public {
  ...
  function exitGem(address manager, address gemJoin, uint cdp, uint amt) public {
    // Moves the amount from the CDP urn to proxy's address
  ...
  function wipe(address manager, address daiJoin, uint cdp, uint wad) public {
    ...
    if (own == address(this) || ManagerLike(manager).cdpCan(own, cdp, address(this)) == 1) {
      ...
      // Paybacks debt to the CDP
      VatLike(vat).frob(
        ilk, urn, address(this), address(this), 0, _getWipeDart(vat, wad * RAY, urn, ilk)
      );
    }
  }
  ...
  function wipeAll(address manager, address daiJoin, uint cdp) public {
    ...
    if (own == address(this) || ManagerLike(manager).cdpCan(own, cdp, address(this)) == 1) {
      ...
      // Paybacks debt to the CDP
      VatLike(vat).frob(ilk, urn, address(this), address(this), 0, -int(art));
    }
  }
  ...
  function wipeAllAndFreeETH(address manager, address ethJoin, address daiJoin, uint cdp, uint wadC)
    public
  {
    address vat = ManagerLike(manager).vat();
    address urn = ManagerLike(manager).urns(cdp);
    ...
    daiJoin_join(daiJoin, urn, _getWipeAllWad(vat, urn, urn, ilk));
    // Paybacks debt to the CDP and unlocks WETH amount from it
    frob(manager, cdp, -toInt(wadC), -int(art));
    // Moves the amount from the CDP urn to proxy's address
    flux(manager, cdp, address(this), wadC);
    // Exits WETH amount to proxy address as a token
    GemJoinLike(ethJoin).exit(address(this), wadC);
  ...
  function wipeAllAndFreeGem(address manager, address gemJoin, address daiJoin, uint cdp, uint amtC)
    public
  {
    address vat = ManagerLike(manager).vat();
    address urn = ManagerLike(manager).urns(cdp);
...
contract DssProxyActionsEnd is Common {
  ...
  function _free(address manager, address end, uint cdp) internal returns (uint ink) {
    bytes32 ilk = ManagerLike(manager).ilks(cdp);
    address urn = ManagerLike(manager).urns(cdp);
  ...
  // Public functions
  function freeETH(address manager, address ethJoin, address end, uint cdp) public {
  ...
  function freeGem(address manager, address gemJoin, address end, uint cdp) public {
    uint amt = _free(manager, end, cdp) / 10 ** (18 - GemJoinLike(gemJoin).dec());
    // Exits token amount to the user's wallet as a token
    GemJoinLike(gemJoin).exit(msg.sender, amt);
  }

  function pack(address daiJoin, address end, uint wad) public {
    daiJoin_join(daiJoin, address(this), wad);
  ...
  function cashETH(address ethJoin, address end, bytes32 ilk, uint wad) public {
  ...
  function cashGem(address gemJoin, address end, bytes32 ilk, uint wad) public {
    EndLike(end).cash(ilk, wad);
    // Exits token amount to the user's wallet as a token
...
contract DssProxyActionsDsr is Common {
  ...
  function exit(address daiJoin, address pot, uint wad) public {
    ...
    DaiJoinLike(daiJoin).exit(msg.sender, bal >= mul(wad, RAY) ? wad : bal / RAY);
  }
