...
contract ClipperTest is DSTest {
  ...
  function assertEqWithinTolerance(uint x, uint y, uint tolerance) internal {
    uint diff;
    if (x >= y) {
      diff = x - y;
    } else {
  ...
  function test_stairstep_exp_decrease() public {
    ...
    uint tic = block.timestamp; // Start of auction
    uint percentDecrease;
    uint step;
    uint testTime = 10 minutes;

    /**
     * Extreme high collateral price ($50m) **
     */

    uint tolerance = 100000000; // Tolerance scales with price
    uint top = 50000000 * RAY;
  ...
  function test_continuous_exp_decrease() public {
    ...
    uint tHalf = 900;
    uint cut = 0.999230132966e27; // ~15 half life, cut ~= e^(ln(1/2)/900)
    calc.file("cut", cut);

    uint top = 4000 * RAY;
    uint expectedPrice = top;
    uint tolerance = RAY / 1000; // 0.001, i.e 0.1%
    for (uint i = 0; i < 5; i++) {
  ...
  function test_linear_decrease() public {
    ...
    uint top = 1000 * RAY;
    uint tic = block.timestamp; // Start of auction
    uint price = calc.price(top, block.timestamp - tic);
    assertEq(price, top);
