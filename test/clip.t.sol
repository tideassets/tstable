...
contract Exchange {
  ...
  constructor(DSToken gold_, DSToken dai_, uint goldPrice_) {
    gold = gold_;
    dai = dai_;
    goldPrice = goldPrice_;
  }

  function sellGold(uint goldAmt) external {
    gold.transferFrom(msg.sender, address(this), goldAmt);
    uint daiAmt = goldAmt * goldPrice / 1e18;
...
contract Trader {
  ...
  function take(uint id, uint amt, uint max, address who, bytes calldata data) external {
    clip.take({id: id, amt: amt, max: max, who: who, data: data});
  }

  function clipperCall(address sender, uint owe, uint slice, bytes calldata data) external {
    data;
...
contract PublicClip is Clipper {
  ...
  function add() public returns (uint id) {
    id = ++kicks;
    active.push(id);
...
contract ClipperTest is DSTest {
  ...
  uint constant goldPrice = 5 ether;

  uint constant startTime = 604411200; // Used to avoid issues with `block.timestamp`

  function sub(uint x, uint y) internal pure returns (uint z) {
    require((z = x - y) <= x);
  }

  function _ink(bytes32 ilk_, address urn_) internal view returns (uint) {
    (uint ink_,) = vat.urns(ilk_, urn_);
    return ink_;
  }

  function _art(bytes32 ilk_, address urn_) internal view returns (uint) {
    (, uint art_) = vat.urns(ilk_, urn_);
    return art_;
  }
  ...
  modifier takeSetup() {
    ...
    assertEq(usr, me);
    assertEq(uint(tic), block.timestamp);
  ...
  function setUp() public {
    ...
    pip.poke(bytes32(uint(4 ether))); // Spot = $2
  ...
  function test_get_chop() public {
    uint chop = dog.chop(ilk);
    (, uint chop2,,) = dog.ilks(ilk);
    assertEq(chop, chop2);
  }
  ...
  function try_kick(uint tab, uint lot, address usr, address kpr) internal returns (bool ok) {
  ...
  function test_bark_not_leaving_dust_rate() public {
    ...
    vat.fold(ilk, address(vow), int(ray(0.02 ether)));
  ...
  function test_Hole_hole() public {
    assertEq(dog.Dirt(), 0);
    (,,, uint dirt) = dog.ilks(ilk);
    assertEq(dirt, 0);

    dog.bark(ilk, me, address(this));

    (, uint tab,,,,) = clip.sales(1);
    ...
    pip2.poke(bytes32(uint(4 ether))); // Spot = $2
    spot.poke(ilk2);

    dog.bark(ilk2, me, address(this));

    (, uint tab2,,,,) = clip2.sales(1);

    assertEq(dog.Dirt(), tab + tab2);
    (,,, dirt) = dog.ilks(ilk);
    (,,, uint dirt2) = dog.ilks(ilk2);
    assertEq(dirt, tab);
    assertEq(dirt2, tab2);
  }
  ...
  function test_partial_liquidation_Hole_limit() public {
    ...
    assertEq(dog.Dirt(), 0);
    (, uint chop,, uint dirt) = dog.ilks(ilk);
    assertEq(dirt, 0);

    dog.bark(ilk, me, address(this));

    (, uint tab, uint lot,,,) = clip.sales(1);
  ...
  function try_take(uint id, uint amt, uint max, address who, bytes memory data)
    internal
    returns (bool ok)
  {
  ...
  function test_take_over_tab() public takeSetup {
    ...
    // Assert auction ends
    (uint pos, uint tab, uint lot, address usr, uint tic, uint top) = clip.sales(1);
    assertEq(pos, 0);
    assertEq(tab, 0);
    assertEq(lot, 0);
    assertEq(usr, address(0));
  ...
  function test_take_bid_recalculates_due_to_chost_check() public takeSetup {
    ...
    (, uint price, uint _lot, uint _tab) = clip.getStatus(1);
    assertEq(_lot, lot);
    assertEq(_tab, tab);
  ...
  function test_take_bid_fails_no_partial_allowed() public takeSetup {
    ...
    assertTrue(
      // partial purchase attempt when !(tab > chost)
      !try_take({id: 1, amt: 1 ether, max: ray(5 ether), who: address(this), data: ""})
    );
  ...
  function test_take_multiple_bids_different_prices() public takeSetup {
    ...
    (, uint _price, uint _lot,) = clip.getStatus(1);
    Guy(bob).take({
      id: 1,
  ...
  function try_redo(uint id, address kpr) internal returns (bool ok) {
  ...
  function test_auction_reset_tail() public {
    ...
    pip.poke(bytes32(uint(3 ether))); // Spot = $1.50 (update price before reset is called)

    (,,,, uint96 ticBefore, uint topBefore) = clip.sales(1);
    assertEq(uint(ticBefore), startTime);
    ...
    (,,,, uint96 ticAfter, uint topAfter) = clip.sales(1);
    assertEq(uint(ticAfter), startTime + 3601 seconds); // (block.timestamp)
  ...
  function test_auction_reset_cusp() public {
    ...
    (,,,, uint96 ticAfter, uint topAfter) = clip.sales(1);
    assertEq(uint(ticAfter), startTime + 1801 seconds); // (block.timestamp)
  ...
  function test_incentive_max_values() public {
    ...
    assertEq(uint(clip.chip()), uint(18.446744073709551615 * 10 ** 18));
    assertEq(
      uint(clip.tip()), uint(6277101735386.680763835789423207666416102355444464034512895 * 10 ** 45)
    );

    clip.file("chip", 2 ** 64);
    clip.file("tip", 2 ** 192);

    assertEq(uint(clip.chip()), 0);
    assertEq(uint(clip.tip()), 0);
  }

  function test_Clipper_yank() public takeSetup {
    uint preGemBalance = vat.gem(ilk, address(this));
    (,, uint origLot,,,) = clip.sales(1);
  ...
  function test_gas_bark_kick() public {
    ...
    assertEq(vat.gem(ilk, me), 960 ether);
    assertEq(vat.dai(ali), rad(1000 ether));
    (uint ink, uint art) = vat.urns(ilk, me);
    assertEq(ink, 40 ether);
    assertEq(art, 100 ether);
