...
contract Usr {
  ...
  function flux(bytes32 ilk, address src, address dst, uint wad) public {
...
contract EndTest is DSTest {
  ...
  function test_cage_snip() public {
    ...
    uint id = dog.bark("gold", urn1, address(this));
    ...
    uint lot1;
    {
      ...
      assertEq(uint(tic1), block.timestamp);
      assertEq(uint(top1), ray(6 ether));
    }
...

    {
      ...
      assertEq(uint(tic2), 0);
      assertEq(uint(top2), 0);
    }
  ...
  function fix_calc_0(uint col, uint debt) internal pure returns (uint) {
    return rdiv(mul(col, RAY), debt);
  }

  function fix_calc_1(uint col, uint debt) internal pure returns (uint) {
    return wdiv(mul(col, RAY), (debt / 10 ** 9));
  }

  function fix_calc_2(uint col, uint debt) internal pure returns (uint) {
    return mul(col, RAY) / (debt / RAY);
  }

  function wAssertCloseEnough(uint x, uint y) internal {
    uint diff = x > y ? x - y : y - x;
    if (diff == 0) return;
    uint xErr = mul(diff, WAD) / x;
    uint yErr = mul(diff, WAD) / y;
    uint err = xErr > yErr ? xErr : yErr;
    assertTrue(err < WAD / 100_000_000); // Error no more than one part in a hundred million
  }

  uint constant MIN_DEBT = 10 ** 6 * RAD; // Minimum debt for fuzz runs
  uint constant REDEEM_AMT = 1_000 * WAD; // Amount of DAI to redeem for error checking

  function test_fuzz_fix_calcs_0_1(uint col_seed, uint192 debt_seed) public {
    uint col = col_seed % (115792 * WAD); // somewhat biased, but not enough to matter
    ...
    uint fix0 = fix_calc_0(col, debt);
    uint fix1 = fix_calc_1(col, debt);

    // how much collateral can be obtained with a single DAI in each case
    uint col0 = rmul(REDEEM_AMT, fix0);
    uint col1 = rmul(REDEEM_AMT, fix1);

    // Assert on percentage error of returned collateral
    wAssertCloseEnough(col0, col1);
  }

  function test_fuzz_fix_calcs_0_2(uint col_seed, uint192 debt_seed) public {
    uint col = col_seed % (115792 * WAD); // somewhat biased, but not enough to matter
    if (col < 10 ** 12) col += 10 ** 12; // At least 10^-6 WAD units of collateral; this makes the fixes almost always non-zero.
    uint debt = debt_seed;
    if (debt < MIN_DEBT) debt += MIN_DEBT; // consider at least MIN_DEBT of debt
    ...
    uint fix0 = fix_calc_0(col, debt);
    uint fix2 = fix_calc_2(col, debt);

    // how much collateral can be obtained with a single DAI in each case
    uint col0 = rmul(REDEEM_AMT, fix0);
    uint col2 = rmul(REDEEM_AMT, fix2);

    // Assert on percentage error of returned collateral
    wAssertCloseEnough(col0, col2);
  }

  function test_fuzz_fix_calcs_1_2(uint col_seed, uint192 debt_seed) public {
    uint col = col_seed % (10 ** 14 * WAD); // somewhat biased, but not enough to matter
    if (col < 10 ** 12) col += 10 ** 12; // At least 10^-6 WAD units of collateral; this makes the fixes almost always non-zero.
    uint debt = debt_seed;
    if (debt < MIN_DEBT) debt += MIN_DEBT; // consider at least MIN_DEBT of debt
