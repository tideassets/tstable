...
contract ClipperMock {
  ...
  function kick(uint, uint, address, address) external pure returns (uint id) {
    id = 42;
  }
}
...
contract DogTest is DSTest {
  ...
  uint constant THOUSAND = 1e3;
  uint constant WAD = 1e18;
  uint constant RAY = 1e27;
  uint constant RAD = 1e45;
  Vat vat;
  VowMock vow;
  ClipperMock clip;
  Dog dog;
  ...
  function setUrn(uint ink, uint art) internal {
    vat.slip(ilk, usr, int(ink));
    (, uint rate,,,) = vat.ilks(ilk);
    vat.suck(address(vow), address(vow), art * rate);
    vat.grab(ilk, usr, usr, address(vow), int(ink), int(art));
    (uint actualInk, uint actualArt) = vat.urns(ilk, usr);
  ...
  function test_bark_partial_liquidation_dirt_exceeds_hole_to_avoid_dusty_remnant() public {
    uint dust = 200;
    vat.file(ilk, "dust", dust * RAD);
    uint hole = 5 * THOUSAND;
    dog.file(ilk, "hole", hole * RAD);
    (, uint chop,,) = dog.ilks(ilk);
    uint artStart = hole * WAD * WAD / chop + dust * WAD - 1;
    setUrn(WAD, artStart);
    dog.bark(ilk, usr, address(this));
    assertTrue(!isDusty());
    (, uint art) = vat.urns(ilk, usr);
  ...
  function test_bark_do_not_create_dusty_auction_hole() public {
    ...
    (,,, uint dirt) = dog.ilks(ilk);
    uint room = hole * RAD - dirt;
    uint dart = room * WAD / rate / chop;
