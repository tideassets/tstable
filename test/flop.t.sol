...
contract FlopTest is DSTest {
  ...
  function test_kick() public {
    ...
    (uint bid, uint lot, address guy, uint48 tic, uint48 end) = flop.bids(id);
    assertEq(bid, 5000 ether);
    assertEq(lot, 200 ether);
    assertTrue(guy == gal);
    assertEq(uint(tic), 0);
    assertEq(uint(end), block.timestamp + flop.tau());
  }
  ...
  function test_yank() public {
    ...
    (uint _bid, uint _lot, address _guy, uint48 _tic, uint48 _end) = flop.bids(id);
    assertEq(_bid, 0);
    assertEq(_lot, 0);
    assertEq(_guy, address(0));
    assertEq(uint(_tic), 0);
    assertEq(uint(_end), 0);
  }
