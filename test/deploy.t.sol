...
contract DssDeployTest is DssDeployTestBase {
  ...
  function testFrobDrawDaiGemLimit() public {
    ...
    colJoin.join(address(this), 1 ether);
    vat.frob(
      "COL", address(this), address(this), address(this), 0.5 ether, 20.454545454545454545 ether
    ); // 0.5 * 45 / 1.1 = 20.454545454545454545 DAI max
  }
  ...
  function testFailFrobDrawDaiGemLimit() public {
    ...
    colJoin.join(address(this), 1 ether);
    vat.frob(
      "COL", address(this), address(this), address(this), 0.5 ether, 20.454545454545454545 ether + 1
    );
  }
  ...
  function testFrobPaybackDai() public {
    ...
    daiJoin.exit(address(this), 60 ether);
    assertEq(dai.balanceOf(address(this)), 60 ether);
    dai.approve(address(daiJoin),type(uint).max);
  ...
  function testFrobFromAnotherUser() public {
    ...
    user1.doFrob(
      address(vat), "ETH", address(this), address(this), address(this), 0.5 ether, 60 ether
    );
  }
  ...
  function testFlip() public {
    ...
    hevm.warp(block.timestamp + ethFlip.ttl() + 1);
  ...
  function testClip() public {
    ...
    col2.mint(1 ether);
    col2.approve(address(col2Join),type(uint).max);
    ...
    (, uint tab, uint lot,,,) = col2Clip.sales(id);
    assertEq(tab, 20 * RAD);
  ...
  function _flop() internal returns (uint batchId) {
    ...
    pipETH.poke(bytes32(uint(300 * 10 ** 18 - 1))); // Decrease price in 1 wei
    spotter.poke("ETH");
    uint48 eraBite = uint48(block.timestamp);
  ...
  function testFlop() public {
    ...
    hevm.warp(block.timestamp + flop.ttl() - 1);
  ...
  function testFlap() public {
    ...
    hevm.warp(block.timestamp + flap.ttl() + 1);
  ...
  function testEnd() public {
    ...
    user2.doFrob(
      address(vat), "COL", address(user2), address(user2), address(user2), 100 ether, 1000 ether
    );
