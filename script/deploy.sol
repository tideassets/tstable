...
contract FlipFab {
  function newFlip(address owner, address vat, address cat, bytes32 ilk)
    public
    returns (Flipper flip)
  {
...
contract DssDeploy is DSAuth {
  ...
  function deployDai(uint chainId) public auth {
  ...
  function deployESM(address gov, uint min) public auth {
  ...
  function deployCollateralClip(bytes32 ilk, address join, address pip, address calc) public auth {
    ...
    // Deploy
    ilks[ilk].clip =
      clipFab.newClip(address(this), address(vat), address(spotter), address(dog), ilk);
    ilks[ilk].join = join;
    Spotter(spotter).file(ilk, "pip", address(pip)); // Set pip
